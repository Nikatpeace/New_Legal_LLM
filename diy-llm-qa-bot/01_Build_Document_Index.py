...
raw_inputs = (
  spark
    .table('nik_demo2_catalog.nik_qabot.sources')
    .selectExpr(
      'Trimmed_Facts',
      'label'
      )
...
long_text = (
  raw_inputs
    .select('Trimmed_Facts') # get just the text field
    .orderBy(fn.expr("len(Trimmed_Facts)"), ascending=False) # sort by length
    .limit(1) # get top 1
     .collect()[0]['Trimmed_Facts'] # pull text to a variable
  )
...
chunked_inputs = (
  raw_inputs
    .withColumn('chunks', get_chunks('Trimmed_Facts')) # divide text into chunks
    .drop('Trimmed_Facts')
